...
def draw_summary_table(canvas, totals, start_position=-8 * cm):
    ...
    summary_data = [
        ...
        ['Montant TTC', format_currency(totals['montant_ttc_10'], 'DH'), format_currency(totals['montant_ttc_20'], 'DH')],
        ...
        ['Total TTC', format_currency(totals['total_ttc'], 'DH'), ''],  # Ligne pour le total général TTC
    ]

    # Création de la table
    summary_table = Table(summary_data, colWidths=[9 * cm, 5 * cm, 5 * cm])  # Ajuster les largeurs des colonnes
    ...
    # Dessiner la table sur le canvas
