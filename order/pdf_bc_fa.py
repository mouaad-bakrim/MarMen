...
def draw_header(canvas, site, document_type):
    ...
    right_margin = 1 * cm
    page_width = A4[0]
    debut_text = 2 * cm  # Valeur par défaut pour debut_text

    # Vérifiez si le logo existe
    if getattr(site.societe_obj, 'logo', None):
        try:
            # Assurez-vous que logo.path est un chemin valide
            with Image.open(site.societe_obj.logo.path) as img:
                width, height = img.size
                ratio = width / height

            # Dessiner l'image dans le PDF
            canvas.drawInlineImage(site.societe_obj.logo.path, 1 * cm, -2.6 * cm, 3 * ratio * cm, 2 * cm)
            debut_text = (2 * ratio + 9) * cm
        except IOError:
    ...
    document_types = {
        ...
        'bon_de_reception': 'Bon de Réception',
        'bon_de_deponse': 'Bon de Déponse',
        'bon_de_livrasion': 'Bon de Livraison',
        'stock': 'Stock',
        'vidange': 'Bon de Livraison',
        'Devis': 'Devis',
        'Movement_export': 'Mouvement de Stock',
        'Réception': 'Bon Réception',
    }
    header_text = document_types.get(document_type, 'Document')

    # Dessiner le texte de l'en-tête
    canvas.setStrokeColorRGB(0, 62 / 255, 97 / 255)  # Bleu
    canvas.setFillColorRGB(0.2, 0.2, 0.2)
    canvas.setFont('Helvetica-Bold', 20)
    canvas.drawString(1 * cm, -4.5 * cm, header_text)
