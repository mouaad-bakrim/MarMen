...
def draw_footer(canvas, site, total_ttc_tva_10=None, total_tva_10=None):
    ...
    canvas.setStrokeColorRGB(0, 62 / 255, 97 / 255)  # Color in RGB
    canvas.setLineWidth(4)
    canvas.line(0, -28 * cm, 21.7 * cm, -28 * cm)  # Line from left to right

    # Affichage des informations de note du site
    note = site.invoice_footer()
    canvas.setFont('Helvetica', 9)  # Font style and size

    # Loop through the note and print each line
    for l, line in enumerate(note):
        text_width = stringWidth(line, 'Helvetica', 9)  # Calculate text width
        # Start text at the calculated position
...
def draw_summary_table(canvas, totals, start_position=-8 * cm, end_position=-10 * cm):
    ...
    summary_table_1.setStyle([
        ...
        ('BACKGROUND', (0, 0), (-1, 0), (0.8, 0.8, 0.8)),
        ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
        ('FONTSIZE', (0, 0), (-1, -1), 10),  # Change from 10 to 9
        ('TEXTCOLOR', (0, 0), (-1, -1), (0.2, 0.2, 0.2)),
